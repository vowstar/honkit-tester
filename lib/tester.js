...
var runHonkitCommand  = function(bookPath, args) {
  logger.info('run command ' + honkitRunnablePath(bookPath) + ' ' + args[0] + ' ' + args[1]);
  ...
  return Q.nfcall(function(next){
    ...
    if (obj && obj.plugins && obj.plugins.length > 0) {
      npm.load(
        ...
        function(err) {
          // install module
          logger.info("Try install gitbook-plugin-" + obj.plugins + ' into ' + bookPath);
          ...
          logger.info("Try install honkit-plugin-" + obj.plugins + ' into ' + bookPath);
