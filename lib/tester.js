...
var runHonkitCommand  = function(bookPath, args) {
  console.error('run command ' + honkitRunnablePath(bookPath) + ' ' + args[0] + ' ' + args[1]);
...
var installHonkit = function(bookPath) {
  return Q.nfcall(function(next){
    try {
      ...
      console.error("Install honkit ...");
      fs.copySync(srcDir, destDir, { overwrite: true, errorOnExist: false }, function (err) {
        if (err) {
          logger.error(err);
          throw new Error('Install honkit failed, file not copy to test tmp dir.');
        } else {
          logger.log("Install honkit success!");
        }
      });
    } catch (error) {
      logger.log("Install honkit failed, try using npm install ...");
      npm.load(
        {
          loglevel: 'silent'
        },
        function(err) {
          // install module
          logger.info('Try install honkit into ' + bookPath);
          npm.commands.install(bookPath, ['honkit'], function(er, data) {
            // log errors or data
            if (er) {
              logger.info('Install honkit failed');
            } else {
      ...
      console.error("Install honkit finished");
      next(null, bookPath);
    }
  }).then(function(){return bookPath;});
};
...
var _readFiles = function(bookPath, extension) {
  ...
  if (!fs.existsSync(target)) {
    console.error('not exist: ' + target);
    deferred.reject();
  }
