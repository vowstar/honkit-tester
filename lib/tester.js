...
var install = function(bookPath) {
  ...
  if (obj && obj.plugins && obj.plugins.length > 0) {
    ...
    child_process.exec('npm install ' + 'gitbook-plugin-' + obj.plugins + ' --prefix ' + bookPath, function(error, stdout, stderr) {
      deferred.resolve(bookPath);
    });

    logger.debug("Try install honkit-plugin-" + obj.plugins + ' into ' + bookPath);
    child_process.exec('npm install ' + 'honkit-plugin-' + obj.plugins + ' --prefix ' + bookPath, function(error, stdout, stderr) {
      deferred.resolve(bookPath);
    });
  }
...
var installHonkit = function(bookPath) {
  ...
  try {
    ...
    child_process.exec('npm install ' + 'honkit' + ' --prefix ' + bookPath, function(error, stdout, stderr) {
      if (error) {
        console.warn(error);
