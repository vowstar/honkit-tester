...
var attachHonkitPluginDir = function(dir) {
  // Honkit only search local folder, so should also link to local.
  var pathToModule = path.normalize(dir);
  var packageJson = require(path.join(pathToModule, 'package.json'));
  var moduleName = packageJson.name;
  var modulePath = path.join(path.dirname(require.resolve("honkit-tester")), '..', moduleName);
  if (!fs.existsSync(modulePath)) {
    console.info('Create symbolic link:' + modulePath);
    fs.symlinkSync(dir, modulePath);
  }
  return dir;
};
