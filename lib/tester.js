...
const installHonkit = bookPath => {
  const target = path.join(bookPath, 'node_modules', 'honkit', 'bin', 'honkit.js');
  const srcDir = honkitModulePath();
  const destDir = path.join(bookPath, 'node_modules', 'honkit');
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // Attempt to install honkit by copying, then use npm to install dependencies
      ...
      fs.copy(srcDir, destDir, { overwrite: true, errorOnExist: false }, async err => {
        if (err) {
          ...
          logger.info("Honkit copied successfully, installing dependencies with npm ...");
          // After copying honkit, run npm install to get all dependencies
          exec(`npm install honkit --prefix ${bookPath}`, async execErr => {
            if (execErr) {
              logger.warn('Honkit dependency installation had issues, but may still work');
            } else {
              logger.info('Honkit dependencies installed successfully');
            }
            try {
              await waitForFile(target);
              resolve(bookPath);
