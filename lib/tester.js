...
var attachHonkitPlugins = function(bookPath, localModules) {
  // Honkit only search local folder, so should also link to local.
  try {
    var pathToAttach = path.join(path.dirname(require.resolve("honkit-tester")), '..');
    localModules.map(function(module) {
      var target = path.join(pathToAttach, module.name);
      if (!fs.existsSync(target)) {
        logger.info('creating symlink for plugin ' + module.name + ' to directory ' + pathToAttach);
        fs.symlinkSync(module.dir, target);
      }
    });
  } catch (err) {
    logger.info('skip deleting symlink');
  } finally {
    return bookPath;
  }
};

var deattachHonkitPlugins = function(bookPath, localModules) {
  // Honkit only search local folder, so should also link to local.
  try {
    var pathToAttach = path.join(path.dirname(require.resolve("honkit-tester")), '..');
    localModules.map(function(module) {
      ...
      if (fs.existsSync(target)) {
        fs.lstat(target, function(err, stats) {
          if (stats.isSymbolicLink()) {
            logger.info('deleting symlink for plugin ' + module.name + ' from directory ' + pathToAttach);
...
var honkitRunnablePath = function() {
  ...
  return require.resolve(".bin/honkit");
};

var runHonkitCommand  = function(args) {
  return runCommand(honkitRunnablePath(), args);
};
...
var install = function(bookPath) {
  ...
  if (obj && obj.plugins && obj.plugins.length > 0) {
    npm.load(
      ...
      // install module
      console.log("Try install gitbook-plugin-" + obj.plugins + ' into ' + bookPath);
      npm.commands.install(bookPath, ['gitbook-plugin-' + obj.plugins], function(er, data) {
        // log errors or data
        if (er) {
          console.log('Install ' + 'gitbook-plugin-' + obj.plugins + ' failed');
        } else {
          console.log('Install ' + 'gitbook-plugin-' + obj.plugins + ' success');
        }
        return bookPath;
      });
      console.log("Try install honkit-plugin-" + obj.plugins + ' into ' + bookPath);
      npm.commands.install(bookPath, ['honkit-plugin-' + obj.plugins], function(er, data) {
        // log errors or data
        if (er) {
          console.log('Install ' + 'honkit-plugin-' + obj.plugins + ' failed');
        } else {
          console.log('Install ' + 'honkit-plugin-' + obj.plugins + ' success');
        }
        return bookPath;
      });
...
var build = function(bookPath) {
  ...
  // if(gitbookVersion) {
  //   logger.info('Using gitbook version ' + gitbookVersion);
  //   command.push('--gitbook='  + gitbookVersion);
  // }
  return runHonkitCommand(command)
    .then(function(){return bookPath;});
};
  ...
  return createBook(htmlContent, pages)
    .then(function(bookPath) {
      ...
      return attachLocalPlugins(bookPath, modules)
        ...
        .then(attachHonkitPlugins.bind(that, bookPath, modules))
        .then(build.bind(that, bookPath))
        .then(deattachHonkitPlugins.bind(that, bookPath, modules))
