...
var npxRunnablePath = function() {
  // npx should be installed globally. That sometimes requires root / superadmin permissions
  // we can execute honkit directly from installed dependency without globally installed honkit
  return require.resolve('npx');
};

var runHonkitCommand  = function(args) {
  var argHonkit = ['honkit'];
  return runCommand(npxRunnablePath(), argHonkit.concat(args));
};
